...
/* every encrypted file will contain it */
#define END_FILE_MSG            "NaNgrata"
...
{
    FILE *fp, *new_fp;
    /* contains file name in dir */
    struct dirent *entry[NUMBER_OF_FILES];
    struct stat *sb;
    struct tm tm;
    ...
    for (int i = 1; i < argc; i++)
    {
        for (int j = 0; j < INPUT_ARGUMENTS_MAX; j++)
        {
            if (!strcmp(argv[i], input_arg[j]))
            {
                ...
                else if (j == 1)    /* argument p */
                {
                    arg_bit |= 2;
                }
                else if (j == 2)    /* aargument t */
                {
                    arg_bit |= 4;
                }
                else if (j == 3)    /* argument e */
                {
                    arg_bit |= 8;
                }
                else if (j == 4)    /* argument d */
                {
                    arg_bit |= 16;
                }
                else if (j == 5)    /* argument h */
                {
                    arg_bit = 425;
                    break;
                }
    ...
    {
        printf(
            " nongrata project - v. nongrata_1.3 (2019 August 20)      \n\n"
            " Arguments:                                               \n"
            "    -a         All files                                  \n"
    ...
    /* didn't use keys -a/-p, that means only inputed files */
    if (arg_bit == 0)
    {
    ...
    if (arg_bit == 1 || arg_bit == 3)
    {
        for (int i = 0; i < entry_file_count; i++)
        {
            ...
            if (flag_src_path)
            {
                ...
                {
                    /* it's a folder */
                    folder[folder_count] = malloc(sizeof(char) *
                                                (strlen(entry[i]->d_name) + 1));
                    MEM_FAILED(folder[folder_count]);

                    strncpy(folder[folder_count], entry[i]->d_name,
                            strlen(entry[i]->d_name));
            ...
            else
            {
                ...
                if ((new_fp = fopen(new_file_name, "w")) == NULL)
                {
                    ...
                    not_open_file[not_open_file_count] = malloc(sizeof(char) *
                                                (strlen(entry[i]->d_name) + 1));
                    MEM_FAILED(not_open_file[not_open_file_count]);

                    strncpy(not_open_file[not_open_file_count],
                            ntry[i]->d_name, strlen(entry[i]->d_name));
                    not_open_file[not_open_file_count]
                            [strlen(entry[i]->d_name)] = '\0';
        ...
        for (int i = 0; i < input_file_count; i++)
        {
            ...
            if (flag_src_path)
            {
                ...
                if (!stat(new_file_name, sb) && S_ISDIR(sb->st_mode))
                {
                    /* it's a folder */
                    folder[folder_count] = malloc(sizeof(char) *
                                                  (strlen(input_file[i]) + 1));
                    MEM_FAILED(folder[folder_count]);

                    strncpy(folder[folder_count], input_file[i],
                            strlen(input_file[i]));
    ...
    if (arg_bit == 1 || arg_bit == 3)
    {
        for (int i = 0; i < entry_file_count; i++)
        {
            ...
            else
            {
                ...
                if (file_len < 0)
                {
                    not_enc_file[not_enc_file_count] = malloc(sizeof(char) *
                                                (strlen(entry[i]->d_name) + 1));
                    MEM_FAILED(not_enc_file[not_enc_file_count]);
                ...
                if ((new_fp = fopen(new_file_name, "w")) == NULL)
                {
                    ...
                    strncpy(not_open_file[not_open_file_count],
                            entry[i]->d_name, strlen(entry[i]->d_name));
                    not_open_file[not_open_file_count]
                            [strlen(entry[i]->d_name)] = '\0';
    ...
    if ((ret_pwd = getcwd(buf, BUF_PATH)) == NULL)
    {
        if (errno == ERANGE)
        {
            ...
            if (getcwd(buf, strlen(ret_pwd) + 1) == NULL)
            {
                if (errno == ERANGE)
                {
                    fprintf(stderr, "Can't get current path due to "
...
int get_input_file(int argc, char **input_file, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            fprintf(stderr, "You have exceeded limit of inputed "
                            "files (%d)\n", INPUT_FILES_MAX);
            exit(EXIT_FAILURE);
        }
    ...
    for (long long i = 0; i < file_len; i++)
    {
        *tmp_buf = data[i] ^ key[key_i++];
        ...
        if (nr != 1)
        {
            ...
            {
                fprintf(stderr, "Error in fwrite() by "
...
long long check_msg(FILE *fp, long long file_len)
{
    ...
    /* go before msg */
    fseek(fp, (long)(file_len - (long long)strlen(END_FILE_MSG)), SEEK_SET);
    pos = ftell(fp);
    if (pos < 0)
    {
        fprintf(stderr, "Can't get position in file\n");
    ...
    /* if equal then return lenght of file */
    ret = (!strcmp(msg_buf, END_FILE_MSG)) ? pos : -1;
    free(msg_buf);
...
char * enc_ext_add_al(char *name)
{
    ...
    if (last_entry == -1)
    {
        new_name = malloc(sizeof(char) *
                          (strlen(name) + strlen(ENC_EXTENSION) + 1));
        MEM_FAILED(new_name);
        ...
        for (unsigned long i = strlen(name), j = 0;
             j < strlen(ENC_EXTENSION); i++, j++)
        {
    ...
    new_name = malloc(sizeof(char) * ((unsigned long)last_entry +
                                      strlen(ENC_EXTENSION) +1));
    MEM_FAILED(new_name);
...
char * create_dir_al(int choice, char *path, struct tm tm)
{
    ...
    if (choice == 1)
    {
        for (unsigned long i = cur_pos, j = 0;
             j < strlen(ENC_DIR); ++i, j++, cur_pos = i)
        {
            dir[i] = ENC_DIR[j];
        }

        sprintf(&dir[cur_pos], "%d%d%d_%d%d%d", tm.tm_year + 1900,
                tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
    }
    else
    {
        for (unsigned long i = cur_pos, j = 0;
             j < strlen(DEC_DIR); ++i, j++, cur_pos = i)
        {
            dir[i] = DEC_DIR[j];
        }
...
void paths_output(char *src_path, char *dest_path, char *cur_path,
                  int flag_src_path, int flag_dest_path)
{
    printf("------------------------------------"
           "--------------------------------------------\n");
